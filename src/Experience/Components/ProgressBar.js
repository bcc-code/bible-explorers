...
export default class ProgressBar {
  ...
  init() {
    ...
    instance.el = {
      ...
      checkpoints: instance.htmlEl.querySelectorAll(
        "[aria-label='checkpoint']:not(:last-child)",
      ),
    };
  ...
  refresh() {
    instance.el.passed.style.width =
      instance.checkpointWidth * instance.program.currentCheckpoint + '%';
    ...
    if (instance.program.currentCheckpoint < instance.el.checkpoints.length) {
      instance.el.checkpoints[
        instance.program.currentCheckpoint
      ].classList.remove('locked');
      instance.el.checkpoints[instance.program.currentCheckpoint].setAttribute(
        'currentCheckpoint',
        '',
      );
    }
  ...
  static HTML() {
    let generatedHTML = `<div class="percentageBar">
                <div class="passed" style="width: ${
                  instance.checkpointWidth * instance.program.currentCheckpoint
                }%"></div>
            </div>
    ...
    for (let i = 0; i < instance.program.totalCheckpoints; i++) {
      if (
        instance.program.programData[i].steps.some(
          (step) => step.details.step_type == 'video',
        )
      ) {
        generatedHTML += `<button class="btn rounded focused" aria-label="checkpoint" data-index="${i}">
        ...
        instance.program.programData[i].steps.some(
          (step) => step.details.step_type == 'quiz',
        )
      ) {
        generatedHTML += `<button class="btn rounded focused" aria-label="checkpoint" data-index="${i}">
        ...
        instance.program.programData[i].steps.some(
          (step) => step.details.step_type == 'pause',
        )
      ) {
        generatedHTML += `<button class="btn rounded focused" aria-label="checkpoint" data-index="${i}">
    ...
    generatedHTML += `<button class="btn rounded focused" aria-label="checkpoint">
